...
app.get('/chat', function(req, res, next) {
    ...
    for (let i = Math.max(0, historyThisUser.length - 3); i < historyThisUser.length; i++) { 
        if (historyThisUser[i].substr(0,1) == ">")
            ...
            messages.push({role:"assistant", content:historyThisUser[i]});
    }
    messages.push({role:"user", content:req.query.input})
    let param = {
        model: 'gpt-3.5-turbo',
        messages: messages,
        max_tokens: 40,
    ...
    request.post({url:"https://api.openai.com/v1/chat/completions", body:JSON.stringify(param), headers:{'Content-Type':'application/json', 'Authorization':'Bearer ' + openAIKey}}, function (err, httpResponse, body) {
        ...
        let last = output.substr(-1);
        if (last != "." && last != "?" && last != "!") output += "."; // because "." is our stop character, it gets swallowed
        console.log("<--- back from openai - " + (replyTimeEnd.getTime() - replyTimeStart.getTime()) + " ms" + " with " + output);
        historyThisUser.push("> " + req.query.input);
        historyThisUser.push(output);
        history[req.query.userid] = historyThisUser;
        finishChat({success:true, output:output}, req, res);
    });
});

function finishChat(o, req, res) {
